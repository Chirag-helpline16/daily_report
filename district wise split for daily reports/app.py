...
def save_uploaded_file(file) -> tuple[str, Path]:
    ...
    file_path = UPLOAD_FOLDER / f"{upload_id}_{timestamp}_{filename}"
    file.save(file_path)
    return upload_id, file_path


def find_uploaded_file(upload_id: str) -> Path:
    if not UPLOAD_ID_RE.match(upload_id or ""):
        raise ValueError("Invalid upload session")
    matches = sorted(UPLOAD_FOLDER.glob(f"{upload_id}_*"))
    if not matches:
        raise FileNotFoundError("Uploaded file session expired. Please choose the file again.")
    return matches[0]


def file_preview(filepath: Path) -> dict[str, Any]:
    df = read_input_file(filepath)
    columns = list(df.columns)
    return {
        "columns": columns,
...
def preview_upload():
    ...
    try:
        ...
        if not allowed_file(file.filename):
            return jsonify({"error": f"Allowed file types: {', '.join(sorted(ALLOWED_EXTENSIONS))}"}), 400

        upload_id, uploaded_path = save_uploaded_file(file)
        preview = file_preview(uploaded_path)
        return jsonify(
            {
                "success": True,
                "upload_id": upload_id,
...
def process_previewed_file():
    uploaded_path: Path | None = None
    try:
        payload = request.get_json(silent=True) or request.form
        upload_id = str(payload.get("upload_id", "")).strip()
        ifsc_column = payload.get("ifsc_column", "__none__")
        pincode_column = payload.get("pincode_column", "__none__")

        uploaded_path = find_uploaded_file(upload_id)
        district_data, unmatched_details, summary = process_input_file(uploaded_path, ifsc_column, pincode_column)

        output_filename = f"IFSC_Pincode_Split_{datetime.now().strftime('%Y%m%d_%H%M%S')}.zip"
        output_path = DOWNLOAD_FOLDER / output_filename
        output_path.write_bytes(create_master_zip(district_data, unmatched_details, summary))
        ...
        return jsonify(
            {
                ...
                "summary": summary,
                "unmatched": unmatched_details[:50],
                "other_count": len(unmatched_details),
                "error_count": 0,
...
def upload_file():
    ...
    try:
        ...
        _upload_id, uploaded_path = save_uploaded_file(file)

        district_data, unmatched_details, summary = process_input_file(
            uploaded_path,
            request.form.get("ifsc_column"),
            request.form.get("pincode_column"),
        )
...
def download_file(filename: str):
    ...
    file_path = DOWNLOAD_FOLDER / safe_name
...
def list_files():
    files = []
    for file_path in DOWNLOAD_FOLDER.glob("*.zip"):
        stat = file_path.stat()
        files.append(
            {
                "name": file_path.name,
    ...
    return jsonify({"files": sorted(files, key=lambda item: item["created"], reverse=True)})
